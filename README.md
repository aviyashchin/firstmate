...
- **Strict project boundary** - the first mate is read-only over your projects except for the narrow exceptions [hard rule 1](AGENTS.md#1-identity-and-prime-directives) authorizes: guarded lifecycle paths such as fleet sync's safe branch pruning, concrete captain-approved operations, and, where a project's own instructions require the work to carry an issue, that project's issue tracker alone; crewmates make every other project change behind the configured merge authority.
