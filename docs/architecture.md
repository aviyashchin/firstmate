...
`bin/fm-dod-lib.sh` is the one owner of that mode's definition of done and of the project-instructions block that carries a project's own committed agent instructions, the `fm/<id>` branch-name carve-out those instructions cannot override, and the mode-specific moment at which a named issue is linked, rendered both into a generated ship brief and into the ship instructions a promoted scout receives, so a promoted worker cannot be handed a weaker contract than a briefed one.
